...
\begin{abstract}
In crowded pedestrian environments where service robots share space with people, local path planners re-decide, at every control cycle, which side --- left or right --- to pass a person on. This paper shows that when this re-decision is made greedily, the passing direction oscillates across cycles (dithering) or the robot freezes, and proposes a decision-layer planner that treats \emph{temporal consistency} of passing decisions as a first-class design objective. There are three key elements. First, the passing relationship with a person is represented as a topological \Class, whose identity is bound to the person's track ID so that cross-cycle correspondence (spawn, removal, merge, split) is stabilized. Second, we introduce a commitment switching rule that unifies the switching margin, dwell, and point of no return into a single \emph{leaky evidence accumulation} equation, with safety layered on top lexicographically. Third, we formalize five properties --- anti-oscillation, switching-rate bound, safety dominance, maneuver completion, and deadlock escape. The guarantee we claim is the \emph{deterministic} switching-rate bound (P2): the interval between two consecutive discretionary switches is bounded below on every sample path, assuming nothing about the distribution of the challenger advantage $D_k$ beyond a range bound, which the $[0,1]$ cost normalization supplies by construction. The stronger probabilistic form of anti-oscillation (P1) --- an upper bound on the expected switching rate per unit time, decaying exponentially through a per-cycle variance-based coefficient $\theta^* \approx 2\Dfloor/(\sigma_D^2\, dt)$ that makes the time discretization $dt$ explicit --- is stated as a \emph{conditional} refinement. It additionally requires a dependence assumption on $\{D_k\}$ (i.i.d., or geometrically mixing); zero mean, bounded variance and bounded range under bare stationarity do not suffice, and we exhibit a stationary counterexample for which the exponential form fails outright. The contribution comprises both design and measured verification. Building on a complete decision rule and five formal properties (P1--P5), we \emph{measure} the decision layer's behavioral consistency, legibility, and cyclic latency using an offline harness that drives the actual decision core (5 scenarios $\times$ 50 seeds; \S5.3, \S5.4): removing the leaky evidence accumulator causes roughly 98 oscillations per encounter under ambiguous ties, whereas the proposed method commits within $\le 1$ (R1 ablation); worst-case cyclic latency at $K=3$ (8 classes) is p99 $58.6\,\mu s$ with a maximum of $651.3\,\mu s$, only about $1.30\%$ of the 20 Hz cycle budget (50 ms), leaving ample real-time margin (R3); and a sensitivity analysis of progress hardening's $\bar\rho$-drive rate identifies a freezing threshold of $v_{\text{lat}} \in (0.20, 0.35)\ \text{m/s}$ (R5). Physical-simulation-based performance metrics (success rate, minimum social distance, collisions), external baselines, and user studies are not measured in this paper; they are specified as a pre-registered protocol and presented as a planned evaluation (\S5.6). The name COMPASS abbreviates \emph{COMmitment-based PASSing for Social navigation}.
...
\item \textbf{Formalization of properties and offline empirical validation.} We state anti-oscillation (P1), switching-rate bound (P2), safety dominance (P3), maneuver completion (P4), and deadlock escape (P5) as propositions. The load-bearing anti-oscillation guarantee is P2: a deterministic lower bound on the interval between consecutive discretionary switches, holding on every sample path under only the range bound $D_k \le D_{\max}$ that the $[0,1]$ cost normalization of \S4.2 supplies by construction. P1 refines this to an exponentially decaying upper bound on the expected switching rate per unit time (and a conditional expected time to switch), with a per-cycle variance-based tuning coefficient $\theta^* \approx 2\Dfloor/(\sigma_D^2\,dt)$ and a factor of $1/dt$ in the per-unit-time bound; we state that refinement as conditional on a dependence assumption on $\{D_k\}$, since stationarity with zero mean and bounded variance admits a counterexample that defeats the exponential form (\S4.6). P4 is proven, on top of the progress $\rho$ dynamics and a lower bound on the lateral progress rate, via a reachable-upper-bound containment condition on the \textbf{challenger accumulator $\Erev$}. Verification is split into two tiers. The decision layer's behavioral consistency, legibility proxy, and cyclic latency (R1, R3, R5) are \emph{measured} using an offline harness that drives the actual decision core, and reported in Chapter 5 (\S5.3, \S5.4); performance metrics requiring physical simulation or a real robot, along with external baselines (R2) and user studies (R4), are left as planned evaluations (\S5.6) specified via a pre-registered protocol.
\end{enumerate}
...
\item \textbf{Asymmetric social cost $J_{\text{social}}$.} A proxemics cost field is laid over each person and integrated for intrusion.
\begin{align}
...
\textbf{Challenger tie-breaking (R6).} If $c'_k = \operatorname*{argmin}$ is not unique (two challengers' normalized costs are tied within numerical tolerance $\varepsilon$), the argmin can waver from cycle to cycle, and the $c' \ne \text{prev}_{c'}$ reset becomes a new source of oscillation that disturbs the accumulator. To prevent this, when tied, we fix a single challenger by the following \textbf{deterministic priority order}.
...
\begin{algorithm}[H]
\caption{COMPASS decision-layer control cycle}
\label{alg:compass}
\begin{algorithmic}[1]
...
\For{each cycle $k$}
  ...
  \Else
    ...
    \State $c' \leftarrow \textsc{TieBreak}(\operatorname*{argmin}_{c \in \mathcal{S}, c \ne c^*} J(c),\ \text{prev}_{c'},\ c^*)$ \Comment{R6 tiebreak ($\varepsilon_{\text{in}}/\varepsilon_{\text{out}}$ hysteresis)}
...
\label{eq:twosided}
\end{equation}
The lower bound ($\Eremax > E_0$) is a \textbf{liveness} condition guaranteeing that discretionary switching is possible at all at $\rho = 0$ (\S4.9), and the upper bound ($\Eremax \le E_0(1+k_\rho)$) is the \textbf{P4 containment} condition that once the maneuver is sufficiently progressed ($\rho \ge \bar\rho$), challenge accumulation is trapped below the threshold and reversal is blocked. Since both requirements are a lower and upper bound on the same variable $\Eremax$, they can be satisfied simultaneously without contradiction. The self-contradiction of v0.3 arose from imposing a separate ``above-threshold headroom'' requirement ($e_{\max} > E_0(1+k_\rho)$) on the same upper bound, which collides head-on with this containment upper bound; as discussed in \S4.9, we clarify that this headroom requirement was never itself a soundness condition and withdraw it.

\begin{proposition}[Anti-oscillation --- probabilistic form, R6] \label{prop:p1}
Suppose the challenger advantage $D_k$ is a stationary stochastic process with mean 0, bounded variance $\mathrm{Var}[D_k] \le \sigma_D^2$, and \textbf{bounded range} $|D_k| \le D_{\max}$ almost surely, and let $\Dfloor > 0$. Then the one-cycle expected increment of the challenger accumulator $\Erev_k$ is bounded above by a negative quantity,
\begin{equation}
\mathbb{E}[(D_k - \Dfloor)\,dt] = (\mathbb{E}[D_k] - \Dfloor)\,dt = -\Dfloor\, dt < 0,
\end{equation}
so $\Erev_k$ is a leaky accumulation process with negative drift, and (i) in ambiguous configurations (mean advantage 0) a discretionary switch requires the fluctuation of $D_k$ to overcome that drift.

Suppose further that $\{D_k\}$ satisfies a \textbf{dependence condition}: either $\{D_k\}$ is i.i.d., or it is geometrically $\alpha$-mixing at a rate fast enough for a regenerative block decomposition with blocks of finite exponential moment. Then (ii) the per-cycle switching probability and the expected switching rate per unit time admit upper bounds that decrease exponentially in $\Dfloor \cdot E_0$.

Part (ii) is \emph{conditional on the dependence condition and is not established under stationarity alone}; see Remark~\ref{rem:dep} for a counterexample. Accordingly the guarantee this paper carries in its headline claims is the \emph{deterministic} switching-rate bound P2 (\S4.7), which requires no distributional hypothesis on $D_k$ beyond the range bound; P1(ii) is a refinement that sharpens rarity from ``bounded frequency'' to ``exponentially rare'' when the dependence condition holds.
\end{proposition}
...
\begin{remark}[Why the dependence condition is necessary] \label{rem:dep}
...
The mechanism is that a single draw persisting forever is perfectly admissible under stationarity, and no amount of bounded support repairs it: the process never forgets its regime, so the block/regenerative argument behind the exponential bound has no independent blocks to work with. This is precisely what the mixing rate in the dependence condition rules out. Note also that this counterexample says nothing about P2 (\S4.7), whose inter-switch interval bound is deterministic and holds on every sample path, including the adversarial one just constructed --- which is why P2, not P1(ii), carries the headline guarantee.
\end{remark}

\begin{proof}[Proof (sketch)]
With $\lambda = 1$ (the most conservative case), $\Erev_k = \sum_{j \le k} (D_j - \Dfloor)\,dt$ subject to the clip, and starting from the lower clip at $0$, a positive threshold $E_0$ must be reached for a switch to occur. Note that the lower clip is a \textbf{reflecting (saturating)} boundary, not an absorbing one: $\Erev$ is held at $0$ only while the increments are non-positive, and leaves $0$ on the first positive increment. Since the expected increment is $-\Dfloor\, dt < 0$, $\{\Erev_k\}$ is an accumulation process with negative drift on a reflecting boundary at $0$.

Here we correct the ``lower bound on the expected hitting time'' statement carried up through v0.2, and with it the justification previously given for discarding that statement. \textbf{With a reflecting lower clip, negative drift does not trap the process}: under the liveness condition $\Eremax > E_0$ (\S4.9) the threshold is reachable, and since $\{\Erev_k\}$ evolves on the bounded state space $[0, \Eremax]$ with a reflecting boundary at $0$, it is positive recurrent and crosses $E_0$ almost surely, so the unconditional first-hitting time $T_{\text{hit}}$ is a.s.\ finite and $\mathbb{E}[T_{\text{hit}}] < \infty$. What makes ``the expected switching time has a finite lower bound'' uninformative is therefore \emph{not} that $\mathbb{E}[T_{\text{hit}}] = \infty$, as stated in earlier versions, but that the design-relevant quantity is the \emph{scaling} of rarity: $\mathbb{E}[T_{\text{hit}}]$ grows exponentially in $E_0$, and a bare lower bound conveys none of that scaling. The core claim of P1 (``switching is rare in ambiguous configurations'') is precisely captured by one of the following two forms.
...
(b) \textbf{(Recommended) Exponential upper bound on switching rate/probability.} By analogy with the Cram\'er--Lundberg inequality of risk theory, the probability that a negative-drift accumulation process starting from $\Erev_0 = 0$ crosses the threshold $E_0$ within one cycle (or unit time) is exponentially bounded above. The adjustment coefficient this argument requires exists here \emph{because of the range hypothesis}: since $|D_k| \le D_{\max}$, the increment $(D_k - \Dfloor)\,dt$ is bounded, its moment generating function is finite for every $\theta$, and the Lundberg equation therefore admits a root $\theta^* > 0$. Equivalently, for i.i.d.\ increments, Hoeffding's inequality applied directly to the bounded increments yields an exponential bound of the same form without invoking risk theory at all. (Under zero mean and finite variance alone no such $\theta^*$ need exist; see the remark on the boundedness hypothesis above.) \emph{This step is where the dependence condition of the statement is consumed}: both the Lundberg root and the Hoeffding bound require independent --- or, via a regenerative block decomposition, asymptotically independent --- increments, and neither survives bare stationarity (Remark~\ref{rem:dep}). Under the dependence condition the per-cycle switching probability $P_{\text{switch}}$ is thus bounded as
\begin{equation}
\label{eq:pswitch}
...
\end{equation}
by a similarly exponential upper bound. Here, so that the adjustment coefficient $\theta^*$ has the units of an inverse of $e$, the variance scale should be matched either to the \textbf{per-cycle variance $\sigma_D^2\, dt^2$} (the variance of the increment $(D_k - \Dfloor)dt$) or, equivalently, to the \textbf{per-unit-time variance $\sigma_D^2\, dt$} --- this normalization explicitly restores the $1/dt$ factor in $\theta^*$ and the switching-rate bound (which was missing in v0.3). That is, switching becomes exponentially rarer as the margin $\Dfloor$ or threshold $E_0$ grows, the variance $\sigma_D^2$ shrinks, and the cycle time $dt$ shortens. If $\lambda < 1$, the leak adds further negative drift, increasing the adjustment coefficient $\theta^*$ and strengthening the bound.
...
If $|D_k| \le \Dfloor$ for all $k$, the increment is always $\le 0$, so $\Erev_k \equiv 0$ and the switching probability is exactly 0 (the v0.1 statement). This probabilistic form relaxes that strong assumption --- from ``every realization is inside the margin'' to ``mean 0, bounded variance, and range bounded by $D_{\max}$'' --- and as $\sigma_D^2 \to 0$ in the bound of (b) above, $P_{\text{switch}} \to 0$, continuously recovering the deterministic case. Note that only the pointwise margin condition $|D_k| \le \Dfloor$ is relaxed; the range bound $|D_k| \le D_{\max}$, being a structural consequence of the cost normalization rather than an assumption about the environment, is retained.
\end{remark}

\begin{remark}[IID assumption and autocorrelation]
The Cram\'er--Lundberg-type bound above, and the standard first-hitting results, are typically derived under the \textbf{IID assumption} on the increments $D_k$. However, $D_k$ in this paper is a stationary process that allows autocorrelation due to the temporal correlation of human motion and prediction. This is exactly the gap the dependence condition of Proposition~\ref{prop:p1} is there to close, and we close it in one direction only. Under geometric $\alpha$-mixing, a regenerative block decomposition recovers the same exponential form with an effective variance and effective adjustment coefficient; the blocks have the finite exponential moments this argument needs because of the range hypothesis $|D_k| \le D_{\max}$, not because of the mixing itself --- mixing supplies the asymptotic independence, boundedness supplies the moments, and \emph{both} are required.

We deliberately do \textbf{not} claim the bound for arbitrary autocorrelated $D_k$. Earlier versions offered a fallback in which $\theta^*$, or the effective variance replacing $\sigma_D^2$, was read as ``absorbing the autocorrelation structure'' --- with the long-run variance $\sum_k \mathrm{Cov}(D_0, D_k)$ substituted for the marginal variance --- while claiming only the qualitative form ``exponential decay in $\Dfloor \cdot E_0$.'' That fallback is withdrawn: a long-run variance does not determine a large-deviation rate, no theorem connects it to the first-passage behaviour of this reflected recursion, and Remark~\ref{rem:dep} exhibits a stationary process for which the qualitative form itself is false. Where the dependence condition cannot be asserted, the guarantee available is P2 (\S4.7), which is deterministic and pathwise. Whether the mixing rate holds for a given deployment is an empirical question, and estimating the autocorrelation structure of $D_k$ from logged encounters is a measurement target of \S5.5 and the planned evaluation (\S5.6). \textbf{Separately, introducing a leak $\lambda < 1$ turns the simple integral accumulation into an OU/AR(1)-type first-hitting problem; the constants (adjustment coefficient, stationary variance) change, but --- still under the dependence condition --- the exponential tail with respect to the threshold $E_0$ is preserved.} The leak therefore affects the constants of part (ii), not the hypothesis it rests on.
\end{remark}

\begin{remark}[$\lambda \cdot \Eth$ interaction]
The interaction of $\lambda$ and $\Eth$ (the leak makes reaching the threshold depend not on the simple integral of $D$ but on its temporal \textbf{shape} --- a short, strong advantage pulse accumulates before leaking away and easily crosses the threshold, whereas a long, weak advantage is offset by leaking and falls short of the threshold) is further analyzed in the O4 section (\S4.9).
\end{remark}
...
\begin{proof}
Immediately after a discretionary switch, $\Erev = 0$, and the maximum one-cycle increment is $(D_{\max} - \Dfloor)dt$. For $\Erev$ to reach $\Eth(\rho) \ge E_0$, at least $\Eth(\rho)/((D_{\max}-\Dfloor)dt)$ cycles are required, giving the above lower bound. The upper clip $e_{\max}$ can only delay or block threshold attainment, never advance it, so this interval bound holds regardless of the clip's size (the headroom-withdrawal argument of \S4.9). Hence switching frequency is bounded and behavior is legible. (Whereas P1(ii) guarantees the \textbf{exponential rarity} of switching in ambiguous configurations only under its dependence condition, P2 deterministically guarantees an \textbf{upper bound on switching frequency} in arbitrary configurations, on every sample path and with no distributional hypothesis --- which is why P2 is the guarantee this paper claims. The bound holds in particular on the stationary counterexample of Remark~\ref{rem:dep}, where P1(ii) does not. Note that, per the drift analysis of \S4.2, the $\Dfloor$ in this bound has a meaning that is constant across cycles only when the normalization constants are fixed as pre-calibrated constants.)
\end{proof}
...
\begin{equation}
\Eth(\bar\rho) = E_0(1+k_\rho \bar\rho^p) > \Eremax \qquad \text{(P4 containment condition)}
\end{equation}
and solving for $\bar\rho$ gives
\begin{equation}
...
\FloatBarrier
\subsection{An observer model of legibility and an operational definition of time-to-legible (R4)}
...
\FloatBarrier
\subsection{Handling clusters, F-formations, and vulnerable pedestrians (O2)}
...
\FloatBarrier
\subsection{$\lambda$ vs. simple dwell timer, and analysis of $e_{\max}$ saturation (O4)}
...
\item \textbf{Shape dependence.} As the core of O4, threshold attainment under leaky accumulation depends not on the simple integral of the advantage but on its \textbf{temporal shape}. A strong advantage arriving as a pulse shorter than the time constant $\tau_{\text{leak}} = -dt/\ln\lambda$ ($\lambda<1$) accumulates before it can leak away and triggers a switch, while a weak advantage spread over a duration longer than $\tau_{\text{leak}}$ is offset by leaking and falls short of triggering a switch. The simple dwell timer has no such notion of a time constant, only a single axis of ``continuous duration.''
...
This section \textbf{reports as results only what was actually measured}. The behavioral consistency, legibility, and per-cycle latency of the decision layer are \textbf{measured} with an offline harness that drives the actual decision core, and are presented in \S5.3 (R1 ablation $\cdot$ R5 $\rho$-sweep) and \S5.4 (R3 latency); the harness setup and metrics are given in \S5.1--5.2, and the sensitivity-analysis design is given in \S5.5. Performance metrics that require physical simulation or a real robot, the external baselines (R2), and the user study (R4) were not measured and are therefore not presented as results; their protocols are specified in full, as a preregistration, in \S5.6 \textbf{Planned Evaluation}. No measured value is fabricated.

\FloatBarrier
\subsection{Offline decision-layer harness --- setup and variants}

The measurements in this section are obtained by driving the \textbf{actual decision core} (\code{DecisionCore}) with the \code{compass\_eval} offline harness across 5 reproducible encounter scenarios $\times$ 50 seeds $\times$ 6 variants ($dt = 0.05\,\text{s} = 20$ Hz, horizon 10 s; CPU AMD Ryzen 5 5500GT). All figures are computed deterministically from the decision stream, and the raw data (\code{compass\_eval/results/ablation\_raw.csv}) are released. What this harness measures is the decision layer's behavioral consistency, legibility proxies, and per-cycle latency; performance metrics that require physical simulation (success rate, collision rate, minimum social distance, lateral jerk) and the external baselines (R2) / user study (R4) are separated into the Planned Evaluation (\S5.6).
...
Legibility (time-to-legible) is computed by applying the \S4.7 Bayesian observer ($\varepsilon = 0.2$, $p^* = 0.9$, right-censored) to the decision stream, and the illegibility rate is the fraction of trials in which the posterior never stabilizes above $p^*$ by the end of the trial (right-censored).
...
Simple dwell (O4)      & 0.80$\pm$0.75 & 0.080$\pm$0.075 & 0.034$\pm$0.031 & 2.26$\pm$3.90 & 50/250 \\
...
\begin{table}[t]
\centering
\caption{Switches/encounter by scenario (mean $\pm$ SD, $N=50$/cell) --- the view in which oscillatory regression is most evident.}
\label{tab:table2}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lccccc}
\toprule
...
Simple dwell (O4)      & 0.00$\pm$0.00 & 2.00$\pm$0.00 & 1.00$\pm$0.00 & 1.00$\pm$0.00 & 0.00$\pm$0.00 \\
...
\FloatBarrier
\subsection{Real-time performance and enumeration-complexity analysis (R3 --- measured)}
...
\begin{itemize}
\item \textbf{$\sigma_{\text{front}}/\sigma_{\text{rear}}/\sigma_s$ sweep.} Each parameter is grid-searched over the range $[0.5\times, 2\times]$ of its baseline value, recording the headline and standard metrics to produce sensitivity surfaces. In particular, we confirm whether increasing $\sigma_{\text{rear}}$ reduces the frequency of rear/blind-spot passing (targeting O2$\cdot$C3).
\item \textbf{Traffic-side (\code{locale.keep\_side}) reversal.} We measure the performance degradation in environments where the right-preference assumption is false (left-hand traffic cultures) or set to \code{none} (targeting the C-questions). A separate performance-sensitivity curve is drawn against the $w_r$ weight.
...
\textbf{Physical simulation and real-robot setup.} Simulation combines Gazebo/Isaac with a reactive pedestrian model (PedSim-family, social-force-based agents) and is repeated across many seeds. Making people reactive is critical to avoiding freezing artifacts. Real-robot validation is performed on a service-robot platform (BungP) with real corridor-encounter cases, together with a demonstration of the presentation layer's FSD-style AR trajectory overlay. \textbf{However, this AR overlay demo is an output of the presentation layer, separate from the decision layer that is this paper's contribution; it is presented only as a qualitative demo and is not included in the quantitative evaluation below (separation of the decision-layer contribution from the presentation-layer demo).} The number of seeds, number of trials, and environment diversity are left as [TBD] but must be stated explicitly when reported. All quantitative comparisons are required to report per-seed variance and confidence intervals (or bootstrap intervals).

\textbf{Controlling simulator circularity (O1, B2).} If $J_{\text{social}}$'s social-force-based cost and the pedestrian simulator (PedSim/social force) share the same assumptions, the proposed method may overfit to the simulator's reaction model. To control for this, we (i) also replace the pedestrian model with a \textbf{different family} than $J_{\text{social}}$ (e.g., ORCA-based, replay-based) to cross-check the robustness of results, (ii) separately report the sim-to-real gap using real-robot encounter cases, and (iii) conduct actual pedestrian-participation experiments where possible. These results are [TBD]; only the design is fixed in this section.

\textbf{Definition of physical performance metrics (standard metrics).} Success rate, integral of minimum social distance/proxemics intrusion, number of near-misses, number of stops/freezing rate, path length, time taken, average speed, human-side disturbance (speed change and detour amount of the reactive person), and lateral jerk, which requires trajectory realization. These are to be filled in as the performance columns corresponding to the measured columns in Table~\ref{tab:table1}, together with the headline metrics of \S5.2; no figures are reported before measurement ([TBD]).

\textbf{Scenarios.} 4 encounter types $\times$ corridor width (narrow/wide) --- head-on confrontation, robot overtaking a person, person overtaking the robot (where the correct behavior for the robot is to hold its lane), and crossing. Additional diagnostic scenarios --- symmetric ambiguous configuration (left/right cost tie, targeting P1), sensor noise/gap flicker (targeting anti-oscillation), narrowing corridor (targeting yield/stop via availability pruning), \textbf{challenger-tie configuration (targeting R6 tie-breaking)}, \textbf{F-formation group encounter (targeting O2: whether the group is not split)}, and \textbf{vulnerable-pedestrian proximity (targeting O2: whether passing is conservative)}.

\textbf{External baselines (R2).} DWB (Nav2's default local planner, a memoryless oscillation baseline), TEB (multi-homotopy + switching cost), MPPI, a social-force-based planner, and ORCA/RVO for congestion. (The internal ablation baselines have already been measured in \S5.1.)
...
\textbf{Legibility user-study protocol (R4).} To validate the observer-model proxy of \S4.7 with human data, a perceptual experiment is designed as follows (results [TBD]).
...
This paper proposes a decision-layer planner for social navigation that treats the temporal consistency of passing decisions as a first-class objective, combining an ID-indexed, cross-cycle-correspondent representation of \Class\ with a leaky-evidence-accumulation-based commitment switching rule and a lexicographic safety override. We formalize five properties (P1--P5). The anti-oscillation guarantee we claim is the deterministic, pathwise inter-switch bound P2, which assumes only a range bound on the challenger advantage; P1 states the sharper probabilistic form --- an upper bound on the expected switching rate per unit time (and the conditional expected switching time), with the tuning coefficient given by the per-cycle-variance-based $\theta^* \approx 2\Dfloor/(\sigma_D^2\,dt)$ and a factor of $1/dt$ in the per-unit-time bound --- as a refinement conditional on a dependence assumption on the challenger advantage, since bare stationarity with zero mean and bounded variance admits a counterexample that defeats the exponential form. We also prove P4 on top of the dynamics of progress $\rho$, a lower bound on lateral progress rate, and a \textbf{containment condition on the challenger accumulator $\Erev$} ($\bar\rho$). In particular, this revision clarifies that the switching dynamics close over the \textbf{single-challenger accumulator} of \S4.3/\S4.5, and, with the two-sided condition on its reachable upper bound $E_0 < \Eremax \le E_0(1+k_\rho)$ --- the lower bound being liveness of discretionary switching and the upper bound being P4 containment --- withdraws the ``above-threshold headroom'' requirement that produced the self-contradiction in v0.3, re-deriving it instead as a liveness condition. On the empirical side, using an offline harness that drives the actual decision core, we measured anti-oscillation behavior (about 98 oscillations per encounter in the ambiguous-tie scenario when the accumulator is removed, vs. 0 for the proposed method), legibility proxies (t-legible, right-censoring rate), and real-time performance ($K=3$ p99 58.6 $\mu$s, about 1.30\% of the 20 Hz budget) (\S5.3, \S5.4), and also obtained, by measurement, grounds for a design improvement in the form of progress hardening's freezing threshold ($v_{\text{lat}} \in (0.20, 0.35)$ m/s).

\textbf{Limitations.} (i) The planar (2D) assumption means multi-level or sloped environments are not handled. (ii) Constant-velocity Kalman prediction is strong in the short term but has limits under abrupt intent changes or multi-modal crowds (a learning-based predictor can be substituted as an interchangeable input, Section 2). (iii) Front/rear passing is handled only as cost and is not separated into distinct \Class{es}. (iv) The empirical scope of this paper is limited to the decision layer. Behavioral consistency, the legibility proxy, and per-cycle latency (R1, R3, R5) were \textbf{measured} with an offline harness driving the actual decision core (\S5.3, \S5.4), but performance metrics requiring physical simulation (success rate, collision, minimum social distance), the external baselines (R2), and the user study (R4) have not yet been measured because the environments have not been built ([TBD]); their protocols have been fixed as a preregistration in the Planned Evaluation (\S5.6). Meanwhile, the ablations showed that the leaky accumulator is the primary oscillation-suppression mechanism (removing $-$hysteresis or $-$hardening alone produced no regression in this regime), and revealed that the $\rho$ drive rate has a freezing threshold ($v_{\text{lat}} \in (0.20, 0.35)$ m/s). The current contribution is therefore the completed design of the decision rule and formal properties together with \textbf{offline empirical validation of decision-layer consistency}; physical performance, external comparison, and human-data validation are to be filled in subsequently according to the protocol of \S5.6.
...
\bibitem{degroot2024topology} O. de Groot, L. Ferranti, D. Gavrila, J. Alonso-Mora. Topology-Driven Parallel Trajectory Optimization in Dynamic Environments. \emph{IEEE Transactions on Robotics (T-RO)}, 41, pp. 110--126, 2024. Also \emph{arXiv preprint arXiv:2401.06021}.
