...
Draw $Z \in \{-a, +a\}$ once, with probability $1/2$ each and $\Dfloor < a \le D_{\max}$, and set $D_k = Z$ for every $k$. The joint law of $(D_{k+1}, \dots, D_{k+n})$ does not depend on $k$, so $\{D_k\}$ is strictly stationary; it has mean 0, variance $a^2$, and range bounded by $D_{\max}$. Yet on the branch $Z = +a$ every increment equals $(a - \Dfloor)dt > 0$, so $\Erev$ rises monotonically and crosses the constant baseline threshold $E_0 < \min(e_{\max},(a-\Dfloor)dt/(1-\lambda))$ in finite cycles under uninterrupted accumulation when $\lambda<1$, with the hitting time in Proposition~\ref{prop:p2live}; the linear ceiling $\lceil E_0/((a-\Dfloor)dt)\rceil$ applies only when $\lambda=1$, while on the branch $Z = -a$ it never leaves $0$. Hence the fixed-cycle marginal $\mathbb{P}(x_k\ge E_0)$ tends to $1/2$ as $k$ grows, with \emph{no} decay in $E_0$ whatsoever --- the per-cycle exponential bound \eqref{eq:pswitch} fails by an arbitrarily large factor.
