...
With $\lambda = 1$ (the most conservative case), $\Erev_k = \sum_{j \le k} (D_j - \Dfloor)\,dt$, and starting from the 0-absorbing barrier (the clip lower bound), a positive threshold $E_0$ must be reached for a switch to occur. Since the expected increment is $-\Dfloor\, dt < 0$, $\{\Erev_k\}$ is an accumulation process with negative drift and a 0-absorbing barrier.

Here we note that the ``lower bound on the expected hitting time'' statement up through v0.2 was inaccurate. \textbf{In a purely negative-drift accumulation process with a 0-absorbing barrier, the expectation of the unconditional first-hitting time $T_{\text{hit}}$ to a threshold $E_0$ is in fact infinite} (because the negative drift traps the process at the 0-absorbing barrier, and sample paths that never reach the threshold occur with positive probability, so $\mathbb{E}[T_{\text{hit}}] = \infty$). Hence a statement of the form ``the expected switching time has a finite lower bound'' is vacuous. The core claim of P1 (``switching is rare in ambiguous configurations'') is precisely captured by one of the following two forms.
...
If $|D_k| \le \Dfloor$ for all $k$, the increment is always $\le 0$, so $\Erev_k \equiv 0$ and the switching probability is exactly 0 (the v0.1 statement). This probabilistic form relaxes that strong assumption to mean 0 and bounded variance, and as $\sigma_D^2 \to 0$ in the bound of (b) above, $P_{\text{switch}} \to 0$, continuously recovering the deterministic case.
\end{remark}
