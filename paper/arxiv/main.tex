...
Here we correct the ``lower bound on the expected hitting time'' statement made by earlier formulations, and with it the justification previously given for discarding that statement. \textbf{With a reflecting lower clip, negative drift does not trap the process}: under the liveness condition $\Eremax > E_0$ (\S4.9) the threshold is reachable, and since $\{\Erev_k\}$ evolves on the bounded state space $[0, \Eremax]$ with a reflecting boundary at $0$, it is positive recurrent and crosses $E_0$ almost surely, so the unconditional first-hitting time $T_{\text{hit}}$ is a.s.\ finite and $\mathbb{E}[T_{\text{hit}}] < \infty$. What makes ``the expected switching time has a finite lower bound'' uninformative is therefore \emph{not} that $\mathbb{E}[T_{\text{hit}}] = \infty$, as stated in earlier versions, but that the design-relevant quantity is the \emph{scaling} of rarity: $\mathbb{E}[T_{\text{hit}}]$ grows exponentially in $E_0$, and a bare lower bound conveys none of that scaling. The core claim of P1 (``switching is rare in ambiguous configurations'') is precisely captured by one of the following two forms.
...
If $|D_k| \le \Dfloor$ for all $k$, the increment is always $\le 0$, so $\Erev_k \equiv 0$ and the switching probability is exactly 0 (the statement of the initial deterministic formulation). This probabilistic form relaxes that strong assumption --- from ``every realization is inside the margin'' to ``mean 0, bounded variance, and range bounded by $D_{\max}$'' --- and as $\sigma_D^2 \to 0$ in the bound of (b) above, $P_{\text{switch}} \to 0$, continuously recovering the deterministic case. Note that only the pointwise margin condition $|D_k| \le \Dfloor$ is relaxed; the range bound $|D_k| \le D_{\max}$, being a structural consequence of the cost normalization rather than an assumption about the environment, is retained.
\end{remark}
