...
**명제 P5 (조건부 유한창 HOLD 전이).** 결정 시각 `τ_i`마다 주기당 최대 한 번의 안전 개입을 기록하고 `C_W(t)=#{i:t−W<τ_i≤t}`로 셀 때, 한 열린-왼쪽 창 안에 `N_thrash`개 개입이 있으면 그 `N_thrash`번째 개입 주기보다 늦지 않게 HOLD로 전이합니다. HOLD는 명시적 외부 해제 전까지 0 명령을 유지합니다.

*논증과 한계.* 임계 사건을 먼저 기록한 뒤 창 크기를 검사하므로 같은 창의 `N_thrash`번째 사건에서 HOLD가 발화합니다. 구현은 동일 결정 시각을 중복 계수하지 않고 `(t−W,t]` 밖 사건을 제거하며, `release_hold()` 호출 전까지 0 목표 명령을 반환합니다. 다만 `W=3 s`에서 `t=0,4,8,…`처럼 드문 사건은 창당 1개뿐이므로 무한히 계속될 수 있습니다. 따라서 P5는 무조건적 교착 탈출·충돌 회피·물리 정지를 보장하지 않는 조건부 가드 계약입니다.
...
**해석 — 강하게 확인된 것.** (i) **진동 제어(핵심 주장).** 누수 누적기를 제거(즉시 argmin)하거나 class 대응을 제거하면 모호 동률에서 조우당 약 98회 진동하는 반면, 제안은 0회로 커밋을 유지합니다(그림 4). 두 분포는 범위가 겹치지 않으므로(제안 전부 0회 vs −누적기 98.16±7.85, 지지집합 비중첩) 분리는 범주적입니다. 이처럼 본 오프라인 실측의 핵심 분리는 범주적이어서 형식 검정이 결론을 바꾸지 않으므로 본 실측에서는 생략하며, Mann-Whitney U 검정·효과 크기·신뢰구간은 계획된 평가(5.6)의 물리 성과 지표에 대해 보고합니다. (ii) **스퍼리어스 억제.** 일시 스파이크에서 제안은 0회 전환(스파이크 무시), argmin은 2회(진입·복귀). (iii) **신속 커밋.** 명확 우위에서 제안은 1회 전환, t-legible 2.37 s. (iv) **가독성.** 진동 변형은 사후확률이 안정되지 못해 자주 비가독입니다(argmin·−class 비가독 각 48/250; `near_tie` t-legible 7.9 s 대 제안 0.05 s).
