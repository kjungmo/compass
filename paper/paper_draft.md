...
상태: 비정본 한국어 설명 초안입니다. 정형 명제·증명·구현 범위의 기준 문서는 [영문 LaTeX 원문](arxiv/main.tex)이며 [PDF](arxiv/main.pdf)는 그 빌드 산출물입니다. 이번 병합 검토에서 핵심 이론·의사코드·관찰자·결과 해석을 기준 원문에 맞춰 정정했습니다. 말미의 개정 이력은 철회된 주장까지 남긴 역사 기록이지 현재 명제나 검증 완료의 근거가 아닙니다. 오프라인 결정 스트림 결과와 선택적 구현의 소프트웨어 시험을 물리·사람 검증과 구분합니다.

-----

## 초록 (Abstract)

혼잡한 보행 환경의 서비스 로봇은 사람을 좌·우 어느 쪽으로 지나갈지 매 제어 주기마다 다시 결정하며, 이 재결정이 탐욕적이면 모호한 구성에서 통과 방향이 주기 간에 진동하거나 로봇이 얼어붙습니다(freezing). 본 논문은 통과 결정의 **시간 일관성(temporal consistency)** 을 1급 설계 목표로 삼는 결정 계층 플래너 COMPASS를 제안합니다. 통과 관계를 사람 track ID에 묶인 위상 `class`로 표현해 주기 간 대응을 안정화하고, 전환 마진·드웰·복귀 불가점을 단일 **누수 증거 누적(leaky evidence accumulation)** 식에 통합한 커밋 전환 규칙 위에 안전을 사전식(lexicographic)으로 얹습니다. 반진동·전환율 상한·안전 지배·기동 완수·조건부 유한창 HOLD 전이의 다섯 속성(P1–P5)을 정형화합니다. 실제 결정 코어를 구동하는 오프라인 하니스(5 시나리오×50 시드)에서, 즉시 argmin 비교 정책은 모호 동률에서 조우당 약 98회 결정 변경을 보이는 반면 제안은 각 시험 조우에서 최대 1회 변경하고, 주기 지연은 K=3(8 `class`)에서 p99 58.6 µs·최대 651.3 µs로 20 Hz 예산(50 ms)의 약 1.30 %입니다. 가독성 프록시와 함께 `v_lat ∈ (0.20, 0.35) m/s`의 **오프라인 전환 봉쇄 구간**을 보고하되, 이를 물리 freezing 임계로 해석하지 않습니다. 물리 성과 지표·외부 비교군·사용자 실험은 측정하지 않았으며, 사전 등록 프로토콜로 §5.6의 계획된 평가에 명세합니다.
...
3. **속성의 정형화와 오프라인 실증.** 핵심 보장은 P2의 재량 전환 간격 하한이며, 전환이 실제 일어난다는 보장이 아니므로 동일 도전자의 지속 선택·가용성, 중단 없는 갱신, 지속 우위 및 clip/leak 헤드룸을 전제로 하는 별도 응답시간 상한과 함께 읽습니다. P1의 지수 확률 상한은 영평균·유계 범위·i.i.d. 입력과 초기 증거 0에 조건부이며, 분산만으로는 성립하지 않습니다. P3는 분기 우선순위, P4는 양의 횡 진행·유계 후퇴 및 잠금 영역 전방 불변성 아래의 완수, P5는 유한창 사건 수 임계에 따른 HOLD 전이입니다. 결정 스트림과 코어 지연은 오프라인 측정이며 물리 안전·목표 성공·사람 가독성은 미검증입니다.

-----

## 2. 관련 연구 (Related Work)

서지와 구체적 선행연구 비교는 [영문 기준 원문 §2](arxiv/main.tex)를 따릅니다. DWA·MPPI·TEB를 모두 동등한 “기억 없는” 정책으로 묶지 않습니다. TEB 구현에는 선택 이력이 있고, Winding Through는 시간에 따른 위상 변경을 비용으로 다루며, de Groot 등의 topology-driven 최적화는 주기 간 위상 일관성을 명시적으로 다룹니다. 따라서 선행연구에 커밋 정형화가 전혀 없었다는 주장은 하지 않습니다.

본 연구가 구분하려는 범위는 track-ID 기반 대응의 설계, 명시적인 단일 도전자 누수 증거 규칙, 조건부 정형 속성, 안전 우선순위의 결합입니다. social momentum은 공동 구성의 각운동량 기반 행동 점수이지 본 구현의 지속 증거 누적기와 같은 상태 변수가 아닙니다. 관련 아이디어를 인용하는 것은 본 관찰자나 물리·사회적 효능이 검증되었다는 뜻이 아닙니다. 본 문서는 서지 수정이 별도로 분기하지 않도록 완전한 문헌 검토를 중복 제공하지 않습니다.

-----

## 3. 문제 정의 (Problem Formulation)

**계층 분리.** 우리는 사회적 주행을 세 계층으로 분리합니다. (i) **결정 계층** — 어느 쪽으로 지나갈지, 양보·진행·추월·정지 등의 이산 의도를 정하는 층. (ii) **궤적 계층** — 정해진 결정을 곡률 연속(G2) 궤적으로 구체화하는 층. (iii) **표현 계층** — 계획된 궤적을 사람에게 시각적으로 드러내는 층(예: AR 오버레이). 본 논문의 기여는 전적으로 **결정 계층**에 있으며, 궤적 계층의 이상적 생성기 계약은 설계 가정이며 현재 선택적 어댑터의 종단간 검증 사실이 아닙니다.
...
**목표.** 제공된 안전·가용성 술어가 재량 전환보다 우선하도록 하고 재량 결정 변경 빈도를 제한합니다. 실제 충돌 회피·물리 진동·사람의 가독성·목표 진행은 분기 우선순위만으로 보장하지 않습니다.
...
이 절의 TTL·생성/소멸·병합/분할·winding-sign 절차는 **의도된 설계 계약**입니다. 현재 코어의 입력 순서 top-K 라벨 열거와 이를 구분하며, 통합 생애주기 또는 tracker 기반 효과를 측정했다고 주장하지 않습니다.
...
**정규화·범위·단위.** 유한한 보정 상수 J_X^max>J_X^min에 대해 Ĵ_X=clip((J_X−J_X^min)/(J_X^max−J_X^min),0,1)를 사용합니다. 유한 비음수 가중치이면 가용 비용은 0≤J=Σw_XĴ_X≤Σw_X이며 |D_k|≤Σw_X입니다. 비가용 무한 비용은 재량 비교에서 제외합니다. 임의의 step_evals 입력이나 unclipped Gaussian 하니스까지 이 범위 보장을 자동 확장하지 않습니다.

고정 보정은 주기 간 정규화 우위의 의미를 유지합니다. 비용 단위를 바꿀 때는 고정 상수도 함께 변환할 수 있으며 그렇게 해야 아핀 불변성이 유지됩니다. 주기별 min-max에서 범위가 좁아지면 같은 Δ_floor를 넘는 데 필요한 원 비용 차이는 더 **작아집니다**. 이는 마진 의미를 바꾸지만, 정규화 D의 범위가 유지되면 P2의 상한은 여전히 성립합니다. P1은 별도의 확률 가정을 요구합니다.

J·D·Δ_floor는 무차원이나 갱신에서 초 단위 dt를 곱하므로 e·E_0·e_max는 정규화 비용×초 단위입니다. λ·k_ρ·ρ·p는 무차원이며 θ*는 증거 단위의 역수라 θ*E_0가 무차원입니다. Δ_floor=.05는 단일 단위가중 정규화 항의 5%이지 총 가중 범위 2.8의 5%가 아닙니다. 단위를 명시하는 이 정정은 노브나 기존 실험 수치를 변경하지 않습니다.
...
2. 없으면 감속하여 시간을 법니다.
3. 그래도 위험하면 정지·양보를 명령합니다. 0 명령은 즉각적인 물리 정지나 움직이는 환경에서의 충돌 회피를 보장하지 않습니다.

**안전 분기 자체의 이력.** 안전 사다리가 1단계(전환)에서 떨면(안전 전환의 고빈도 반복) 그 자체가 새 진동원이 됩니다. 이를 막기 위해 안전 분기에도 최소 이력을 부여합니다. (i) **안전 드웰.** 안전 전환 직후 짧은 보호 구간 `T_safe_dwell` 동안에는 추가 안전 전환을 막고, 위반 지속 시 곧장 2단계(감속)로 떨어집니다. 즉 "한 번 안전 전환을 시도했는데 곧바로 다시 안전 위반"이면 1단계에서 다시 떨지 않고 **감속 우선**으로 전이합니다. (ii) **감속 우선 조건.** 현재 rolling window 내 안전 개입 횟수가 임계(아래 N_thrash의 부분 임계)를 넘으면 1단계를 건너뛰고 2단계부터 시작합니다. 해당 조건이 활성인 동안 반복 안전 전환을 억제하지만, 창 만료·환경 변화까지 포함한 전역적 단조 하강을 보장하지 않습니다.

**스래시 가드.** 부적합한 현재 커밋에 대한 안전 분기 호출을 안전 개입으로 셉니다. 실제 class 전환 없는 감속도 포함합니다. 같은 결정 시각에는 한 번만 기록하고 열린-왼쪽 창 `(t−W,t]`의 개입 수가 `N_thrash`에 도달하면 HOLD에 들어갑니다. HOLD는 명시적 외부 해제까지 0 명령을 유지하며 교착 회복을 보장하지 않습니다. `release_hold()`는 모드·창·드웰만 해제하고, 새 커밋 및 진행 재앵커는 별도 연산입니다.

### 4.5 통합 알고리즘과 궤적 인터페이스

    상태: c*, e, ρ, L_real, L_plan, 안전 개입 시각 deque Q, prev_c′, 드웰 만료, mode
    매 주기 k (현재 시각 t_k, 검증된 간격 h_k):
      1) 𝒞 ← class 열거; J(c)와 안전·가용 집합 𝒮 평가
         if mode == HOLD: 0 명령 출력; continue  # 외부 해제까지 유지
      2) if c* ∉ 𝒮:
           Q에 현재 시각 한 번 기록; τ ≤ t_k−W 제거; n_thrash ← |Q|
           if 드웰 중 or n_thrash ≥ max(1, floor(N_thrash/2)):
             v_target ← max(0, v_in−a_brake·h_k)
           elif 𝒮 ≠ ∅:
             c* ← TIEBREAK(𝒮); e,ρ,L_real←0; 드웰 만료←t_k+T_safe_dwell
           else: v_target ← max(0, v_in−a_brake·h_k)
           감속 분기에서 TTC<TTC_stop이면 mode←STOP
           if n_thrash ≥ N_thrash:
             mode←HOLD; v_target←0
             𝒮가 비지 않으면 그 최소비용 class 선택, e·ρ·L_real·드웰 리셋
           안전 결과 출력; continue  # 안전 분기는 진행도 갱신 생략
      3) if 𝒮에서 c*를 뺀 도전자 집합이 비지 않음:
           c′ ← TIEBREAK(도전자 집합, prev_c′, c*)
           D ← J(c*)−J(c′)
           if c′ ≠ prev_c′: e←0
           e ← clip(λe+(D−Δ_floor)h_k, 0, e_max)
           if e ≥ E_0(1+k_ρρ^p): c*←c′; e,ρ,L_real←0
           prev_c′←c′
      4) 명시된 측정 변위 또는 legacy 속도 프록시 계약으로 진행 갱신
         {c*, v_target, mode, 안전 속도 상한}을 명령 어댑터에 출력

![그림 3. COMPASS 결정 계층의 제어 주기별 흐름도](figures/fig_decision_flow.png)

*그림 3. 결정 계층의 개념 블록도이며 실제 순차 실행 흐름을 그대로 그린 것이 아닙니다. 안전이 재량 증거보다 우선하고, 임계 시험 전 증거 x와 전환 후 저장 증거 q를 구분합니다. P5는 열린-왼쪽 rolling window의 안전 개입 수에 따른 조건부 HOLD 계약입니다. 정확한 실행 순서는 위 의사코드와 구현을 따릅니다.*

구현은 ROS2 `Nav2`([Macenski et al. 2020])의 `nav2_core::Controller` 플러그인(`compass_nav2::CompassController`)이며, `controller_server`가 호스팅합니다. 결정 1주기는 `computeVelocityCommands` 호출당 1회 수행되고, 주기율은 `controller_server`의 `controller_frequency` 설정값(예제 구성은 20.0 Hz)이 정하며, costmap은 `Costmap2DROS`에서, 사람 트랙은 `/people`(`compass_msgs/People`) 구독으로 받습니다. 플러그인은 `Nav2` lifecycle 규약(`configure`/`activate`/`deactivate`/`cleanup`)을 준수하고 출력은 `TwistStamped`이며, 대응 상태(라벨·e·ρ·prev_c′·안전 드웰·개입 창 상태)를 주기 간 유지합니다.
...
**결정 계층의 시스템 인터페이스.** 아래 표는 연결된 인터페이스 규약을 요약합니다.
...
| 전역 계획·현재 pose | `nav_msgs/Path` (`setPlan`) 및 현재 pose | 비지 않은 계획 header와 pose header는 모든 모드에서 costmap 전역 frame과 일치해야 하며, 불일치 시 0 명령 반환(묵시적 변환 없음) | 새 계획 수신 시 교체 | — | 최신 계획 유지 |
...
정확한 명제·증명과 반례는 [기준 영문 LaTeX](arxiv/main.tex)의 P1–P5 및 응답시간 명제를 따릅니다. 여기서는 같은 조건을 요약합니다. e^rev는 별도 상태가 아니라 실제 전환을 결정하는 단일 도전자 증거 e입니다. 구현에 남은 e_fwd는 전환 조건을 결정하지 않습니다.

**P1 — 조건부 주기별 교차확률.** D_k가 영평균이고 |D_k|≤D_max이며 0<Δ_floor<D_max이면 원시 입력 X_k=(D_k−Δ_floor)dt의 평균은 음수입니다. 이는 반사 clip을 거친 상태 증가량의 평균이 음수라는 말이 아닙니다. 초기 증거 0, 연속 증거 갱신 주기와 i.i.d. 입력을 추가로 가정합니다. 갱신 전 저장 증거 q_{k−1}, 리셋 후 r_k∈[0,q_{k−1}], 임계 시험 전 x_k=clip(λr_k+X_k,0,e_max)를 구별하면, 전환 후 저장 q_k가 0이어도 P(x_k≥E_0)≤exp(−θ*E_0)입니다. θ*>0은 E[exp(θ*X_1)]=1의 정확한 양의 근입니다. 증거를 Lindley 재귀로 지배하고 i.i.d. 순서 반전과 최대 부등식을 사용합니다. 양의 입력이 없으면 전환은 0입니다. 단위시간당 기대율에는 1/dt가 곱해집니다.

θ*≈2Δ_floor/(σ_D²dt)는 작은 드리프트·준가우시안 **튜닝 근사일 뿐 보장 계수가 아닙니다.** 정상성·분산만으로 지수 상한은 성립하지 않으며, 자기상관을 장기 분산에 흡수하면 된다는 이전 주장은 철회합니다. Z=±a를 한 번 뽑아 D_k=Z로 고정한 정상 과정에서는 λ=1, ρ=0, E_0=n(a−Δ_floor)dt, e_max≥E_0일 때 첫 교차 주기 n의 확률이 1/2입니다. n을 늘리면 한 시점 주변법칙의 Lundberg 근으로 만든 지수 상한과 모순됩니다. λ<1이면 선형 ceiling 대신 아래 기하 누적 응답식을 사용해야 합니다. 실제 reset-on-switch 과정의 주변확률이 1/2로 수렴한다는 주장은 하지 않습니다. 데이터 의존적 갱신 생략 후 남은 부분열에도 i.i.d. 보장을 자동 이전하지 않습니다.

**P2 — 재량 전환 간격 하한.** b=D_max−Δ_floor>0, D_k≤D_max, 0<λ_k≤1이고 전환 후 증거가 0이며 다른 연산도 증거를 더하지 않는다고 합니다. 연속 재량 전환 주기 s<t에서 전환 **전** 문턱 E_t=E_th(ρ_t)에 대해 Σ_{k=s+1}^t h_k≥E_t/b입니다. 고정 dt이면 t−s≥ceil(E_t/(b dt))≥ceil(E_0/(b dt)). E_0>b dt여야 최소 2주기의 비공허한 하한입니다. 가변 간격에서는 검증된 h_max가 있어야 dt 대신 h_max로 주기 수를 제한할 수 있습니다. Nav2의 설정 20 Hz만으로 고정 간격은 입증되지 않습니다. 기본 D_max=Σw=2.8일 때 최소 3주기, D_max≤1일 때 7주기입니다. 하한은 반올림 전 문턱에 경화를 적용합니다. 안전·생애주기 변경, 물리 진동, 사람 가독성은 이 명제의 대상이 아닙니다. N주기 관측창에는 최대 1+floor((N−1)/N_0)회의 경계항이 필요합니다.

**별도 명제 — 지속 선택된 도전자에 대한 조건부 응답시간.** 같은 가용 도전자가 선택되고 안전 개입·HOLD·retarget·외부 리셋 없이 매 주기 갱신·임계 시험을 받으며 D_k≥D_sus>Δ_floor, E_th(ρ_k)≤E_bar≤e_max라고 합니다. 고정 dt·λ에서 a=(D_sus−Δ_floor)dt이고 λ<1이면 a/(1−λ)>E_bar를 요구합니다. 그러면 N_resp=ceil(log(1−E_bar(1−λ)/a)/log λ) 이내에 전환합니다. λ=1이면 ceil(E_bar/a)입니다. 모든 정당한 대안에 대한 무조건 응답·recall 보장은 아닙니다. 반대로 초기 증거 q<L, 문턱≥L, (D_bar−Δ_floor)dt≤(1−λ)L이면 증거가 L 미만에 머뭅니다. 문턱>e_max인 주기는 입력 크기와 무관하게 봉쇄됩니다. clip과 문턱의 등호는 도달 가능할 수 있지만, 누수 평형 등호는 아래에서 유한 주기 내 도달하지 않습니다.

**P3 — 안전 지배.** 활성 결정 주기에서 c*∉𝒮이면 누적·진행과 무관하게 안전 분기가 재량 분기보다 우선합니다. HOLD에서는 이미 0 명령을 유지합니다. 이는 분기 우선순위 계약이지 물리 충돌 회피 보장이 아닙니다.

**P4 — 점별 봉쇄와 조건부 완수.** 고정 dt·λ, 초기 증거 0과 유계 D 아래 도달 상한은 M=min(e_max,(D_max−Δ_floor)dt/(1−λ))이며 λ=1이면 M=e_max입니다. E_0<M<E_0(1+k_ρ), k_ρ>0, p≥1이면 ρ_bar=((M/E_0−1)/k_ρ)^(1/p)∈(0,1)입니다. **ρ>ρ_bar인 주기**에는 문턱이 M보다 크므로 전환할 수 없습니다. clip에 도달 가능한 등호 경계는 잠금에 포함하지 않습니다. 누수 평형은 아래에서 점근적으로 접근하므로 clip과 누수 평형의 등호도 다르게 처리합니다. 하한 E_0<M은 입력 범위 아래의 대수적 헤드룸이지 지속 우위·선택 없이 실제 전환이 발생한다는 뜻이 아닙니다.

안전 개입이 없고 같은 계획 길이 L_plan>0의 중단 없는 커밋에서 L_real,K≥K v_lat,min dt−B, v_lat,min>0, Σb_k≤B<L_plan이며, 잠금에 진입한 뒤 ρ가 다시 ρ_bar 이하로 내려가지 않는 **전방 불변성**까지 가정하면 0 진행 시작부터 최대 **ceil((L_plan+B)/(v_lat,min dt))** 주기 안에 완수합니다. 잠금 이전 전환은 가능하며 새 커밋에서는 보장을 다시 시작합니다. 진입 자체나 전방 불변성을 보장하지 않는 일반 반복 커밋에는 무조건 완수 정리를 주장하지 않습니다.

**P5 — 조건부 유한창 HOLD 전이.** 결정 시각 τ_i마다 최대 한 번의 안전 **개입**을 기록하고 C_W(t)=#{i:t−W<τ_i≤t}로 셉니다. 현재 사건을 기록한 뒤 임계를 검사하여 한 창에 N_thrash개 개입이 생기면 그 사건 주기까지 HOLD에 들어갑니다. HOLD를 유지하고 어댑터가 집행하면 외부 해제까지 0 명령입니다. W=3초에서 0,4,8,…초에 생기는 희소 사건은 무한히 지속할 수 있으므로 무조건적 교착 탈출·복구·물리 정지·충돌 회피는 보장하지 않습니다.
...
이 절은 외부 인지와 향후 정책 확장에 대한 **설계**이며, 현재 옵션의 종단간 구현·검증 완료를 나타내지 않습니다. P5의 안전 사건 창만으로 지속 무전환/freezing을 검출하지 못하므로, 시간 기반 검출과 실측 검증은 별도 요구사항입니다.
...
**보수적 기본값의 freezing 부작용 한정.** 보수적 노브 상향은 전환을 과도하게 늦출 수 있으므로 향후 정책에서는 유한 상한과 지속 무진행 검출을 별도로 설계·평가해야 합니다. P5의 유한창 안전 개입 가드는 지속적인 재량 무전환을 감지하지 않으며, 그 자체로 freezing 경계를 검출하거나 안전·가독적 정지 및 교착 회복을 보장하지 않습니다.
...
단순 드웰은 연속 우위의 길이를 재고 끊기면 리셋하지만 누수 증거는 우위의 크기와 시간적 형태를 함께 반영합니다. 포화는 P2의 간격 하한을 깨지 않으며 오히려 전환을 늦추거나 막습니다. 다만 실제 응답은 §4.6의 지속 선택·입력 하한·clip 및 누수 평형 헤드룸을 요구합니다. 기본값에서는 ρ>sqrt(2/3)≈.8165이면 문턱이 .50 clip보다 커 입력과 무관하게 봉쇄됩니다. D=.25, .40, .50, 1.0, ρ=0의 고정 문턱 응답식은 각각 76, 24, 17, 7주기입니다.

E_0<e^rev_max<E_0(1+k_ρ)는 입력 범위 아래의 초기 헤드룸과 **열린** 잠금 영역의 공존을 위한 설계 조건입니다. 실제 입력이 충분히 지속될 것을 보장하지 않으므로 이를 무조건 가동성으로 읽지 않습니다. e_max=E_0이면 clip 도달로 전환할 수 있으나, 누수 평형=E_0일 때는 아래에서 유한 exact-real 갱신으로 도달하지 못합니다. 등호를 포함한 이전의 일괄적인 “전환 불가” 또는 “ρ≥ρ_bar에서 잠금” 주장은 철회합니다.
...
가독성(time-to-legible)은 운동학적 궤적 대신 **출력된 결정 라벨 스트림**에 관찰자를 적용한 프록시입니다. 실제 출력 라벨에 별도 잡음을 추출해 섞는 것이 아니라 대칭 오독 우도 ε=.2, p*=.9, 사전확률 .5를 고정합니다. log-odds 갱신으로 포화 오류를 막고, 최종 출력 측을 가설 목표로 하여 그 확신이 지평 끝까지 유지되는 suffix의 최초 시점을 찾습니다. 이 시점 이후 **최소 0.30초의 실제 관측 후속 구간**이 있어야 도달로 인정하고, 마지막 tick 및 더 짧은 유지구간은 검열합니다. 이는 사람 가독성·독립적 운동 지표가 아닙니다.

**입력·계수·관찰자 버전.** 모든 여섯 정책은 관측된 연속 출력 사이의 변경만 셉니다: sum_{i=1}^{n−1}[s_i≠s_{i−1}]. 초기화 상태에서 첫 출력으로의 변경은 모두 동일하게 제외하므로 clean_commit에서 argmin의 0은 초기 결정이 없었다는 뜻이 아닙니다. 원 하니스의 unclipped Gaussian 비용 잡음은 거의 확실한 유계 입력이 아니므로 P1/P2의 유계 입력 가정 검증과 별개이며, 유계 속성 시험은 별도 소프트웨어 증거입니다. 원 main 9fe495a의 probability-space 결과, PR12 5343d457의 log-odds-only 수치 교정, 현행 log-odds+0.30초 follow-up 결과를 구분합니다. 마지막 단계는 수치 구현뿐 아니라 지표 정의 변경입니다. 결정 스트림이 같아도 관찰자 출력 CSV 열은 같지 않으며 서로 다른 버전을 섞어 집계하지 않습니다.

**비교군의 범위.** −hysteresis와 −progress hardening은 코어 노브만 바꾼 ablation입니다. 즉시 argmin·단순 드웰은 별도 정책이며, 역사적 CSV key −class correspondence는 |J_R−J_L|<.10이면 동전 추출, 그 밖에는 argmin인 **합성 label-flicker 스트레스 비교군**입니다. 이 비교군은 누적기도 없으므로 대응만 제거한 인과 ablation이 아닙니다.
...
| :- | :- | :- | :- | :- | :- |
| 전체(제안) | **0.40±0.49** | **0.040±0.049** | **0.018±0.022** | **1.33±1.66** | 0/250 |
| −히스테리시스 | 0.40±0.49 | 0.040±0.049 | 0.018±0.022 | 1.00±1.20 | 0/250 |
| −진행 경화 | 0.40±0.49 | 0.040±0.049 | 0.018±0.022 | 1.31±1.65 | 0/250 |
| −누적기(argmin) | 29.74±36.70 | 2.974±3.670 | 0.420±0.374 | 3.60±4.64 | 72/250 |
| 단순 드웰(0.6 s) | 0.80±0.75 | 0.080±0.075 | 0.034±0.031 | 2.26±3.90 | 50/250 |
| −class 대응 | 37.20±36.12 | 3.720±3.612 | 0.515±0.401 | 3.60±4.63 | 71/250 |
...
*그림 4. 변형별 조우당 class 전환 횟수 (평균±SD, 로그 스케일; N=250 회/변형, 5개 시나리오×50 시드, 지평 10.0 s). 즉시 argmin 정책과 합성 label-flicker+argmin 비교군은 조우당 각각 29.74회·37.20회로 진동하는 반면, 제안(full)은 0.40회로 사실상 커밋을 유지합니다. −히스테리시스·−진행 경화 단독 제거는 본 시나리오군에서 퇴행이 관찰되지 않아, 해당 시나리오에서 누적기 설정의 잡음 억제와 정합합니다. 합성 비교군은 대응 제거의 단독 인과 효과를 식별하지 않습니다. 원자료: `src/compass_eval/results/ablation_raw.csv` (실측, 날조 없음).*

**해석 — 강하게 확인된 것.** (i) **진동 제어(핵심 주장).** 즉시 argmin 정책과 합성 label-flicker 스트레스 비교군은 모호 동률에서 조우당 약 98회 진동하는 반면, 제안은 0회로 커밋을 유지합니다(그림 4). 관측된 표본 범위는 겹치지 않으므로(제안 전부 0회 vs −누적기 98.16±7.85, 관측 범위 비중첩) 이 시험 표본에서는 완전히 분리됩니다. 분포의 지지집합이나 대응의 인과 효과를 증명한 것은 아닙니다. 이처럼 본 오프라인 실측의 핵심 분리는 범주적이어서 형식 검정이 결론을 바꾸지 않으므로 본 실측에서는 생략하며, Mann-Whitney U 검정·효과 크기·신뢰구간은 계획된 평가(5.6)의 물리 성과 지표에 대해 보고합니다. (ii) **스퍼리어스 억제.** 일시 스파이크에서 제안은 0회 전환(스파이크 무시), argmin은 2회(진입·복귀). (iii) **신속 커밋.** 명확 우위에서 제안은 1회 전환, t-legible 2.37 s. (iv) **가독성.** 진동 변형은 사후확률이 안정되지 못해 자주 비가독입니다(argmin·합성 −class 검열은 각각 72/250·71/250; `near_tie` t-legible 7.9 s 대 제안 0.05 s).

**해석 — 사전 가설과 달랐던 것(측정에 의한 정정).** −히스테리시스(Δ_floor=0)와 −진행 경화(k_ρ=0)는 본 시나리오들에서 제안 대비 **퇴행이 관찰되지 않았습니다.** 누수 누적기의 임계 `E_0`와 누수가 이미 영평균 잡음을 기각하므로, 본 동작 영역에서 1차 진동 억제 기제는 Δ_floor·k_ρ 가 아니라 **누적기** 자체입니다. 즉 초기 개정본이 사전 등록한 "−히스테리시스 → 전환수 폭증" 가설은 누수 누적기 설계에서는 성립하지 않으며, 이는 ablation 측정이 제공한 **가설 정정**입니다. 또한 `intermittent`에서 제안은 1.55~2.10초에 한 번 전환하고, log-odds로 계산한 동일 관찰자에서 평균 t-legible 4.11초, 검열 0/50입니다. 기존 50/50 검열은 확률 갱신의 부동소수점 포화 오류였으며, 지평 끝의 늦은 전환이라는 해석은 철회합니다. 같은 시나리오에서 단순 드웰은 간헐 우위가 타이머를 계속 리셋해 정당한 전환을 끝내 못 하지만(전환 0), 초기 `class`에 머무른 결과 관찰자에게는 즉시 가독(0/50 검열, t-legible 0.05 s)입니다 — 가독성 수치만으로는 전환 실패가 가려지는 사례입니다. 단순 드웰의 전면 검열(50/50)은 `intermittent`가 아니라 `mid_reversal`에서 발생하며(중간 전환 후 사후확률이 지평 끝까지 재안정화되지 못함), 같은 시나리오에서 제안은 커밋을 유지해(전환 0) 0/50 검열로 즉시 가독입니다. 다만 `mid_reversal`에서 제안의 무전환은 위 단순 드웰 사례와 동일한 가림 효과의 이면입니다 — 기본 노브에서 누적기가 지평 내 후기 조우 역전(시나리오 스크립트상 도전자 우위가 지평 끝에 +0.25 = 5×`Δ_floor`까지 지속 상승)을 따라잡지 못한 것으로, 4.3이 명명한 보수성 비용 및 아래의 오프라인 전환 봉쇄 민감도와 정합하는 트레이드오프입니다. 끝으로 본 절의 t-legible·검열률은 5.1의 결정 스트림 프록시 관찰자가 산출한 값으로, 결정 스트림의 안정성을 측정한 것이지 독립적 운동 가독성 축이 아님을 다시 밝혀 둡니다.

**ρ-율 민감도와 오프라인 전환 봉쇄.** 스크립트형 `clean_commit` 결정 코어 하니스에서 진행 입력 `v_lat`을 0.05→0.50 m/s로 올리면, `v_lat ≤ 0.20 m/s`에서는 제안이 항상 커밋(50/50, t-legible≈2.4 s)하지만 **`v_lat ≥ 0.35 m/s`에서는 전환이 봉쇄(0/50)** 됩니다. 따라서 측정된 것은 **오프라인 전환 봉쇄 구간 `v_lat ∈ (0.20, 0.35) m/s`** 입니다. 이는 스크립트 입력과 기존 `v_lat=|v_cmd|` 근사의 민감도이며, 로봇의 실제 정지·물리 freezing·목표 실패를 입증하지 않습니다. 결과는 측정 cross-track 진행을 분리할 설계 근거로 사용합니다(`compass_eval/results/R5_rho_sweep.md`).

**재현.** 새 ROS 작업공간에서는 `colcon build --base-paths src --packages-up-to compass_eval`로 종속 패키지를 함께 빌드합니다. `./build/compass_eval/compass_eval ablation /tmp/compass_rerun`은 별도 CSV와 stdout 요약을 만들며 latency·rho_sweep은 stdout을 냅니다. 그림 생성기는 `paper/figures/scripts`에 있습니다. 역사적 latency는 머신·도구체인 의존이며 정확히 같을 것으로 기대하지 않습니다. 보존된 결과를 덮어쓰지 말고 유지되는 재현 명령·버전은 README 및 증거 provenance를 따릅니다.

### 5.4 실시간성·열거 복잡도 분석 (실측)

**열거 복잡도(해석적 한계).** 현재 코어는 K=min(사람 수,K_cap)에서 2^K개의 라벨을 열거하고 각 비용을 평가한 뒤 안전 집합을 만듭니다. 기본 K_cap=3에서 최대 8개라는 구성 상한이며, 영향도 순위·그룹 병합이나 특정 가지치기 분포의 구현 증거가 아닙니다. 후보당 비용은 rollout 표본·사람 수·환경 질의에 의존합니다. 실제 장면의 |𝒮_k| 분포는 미측정이며, 현재 루프가 안전 후보만 평가한다고 해석하지 않습니다.

**K_cap 운영 규칙.** 현재 구현은 입력 순서의 첫 K_cap명을 명시 라벨 차원으로 쓰며 기본값은 3입니다. 영향도 순위나 초과 인원의 coarse class 병합은 구현하지 않습니다. 비용·안전 질의가 나머지 입력 사람도 고려할 수 있으나 이들의 통과 측 실현을 보장하지는 않습니다.

**역사적 유한 batch 시간 측정.** 보존된 `compass_eval latency`는 AMD Ryzen 5 5500GT with Radeon Graphics에서 조건당 20,000회 `DecisionCore::step`을 측정했으며, 열거한 모든 2^K 라벨과 all-safe 환경 fixture를 사용했습니다. 관측된 p99/최대는 K=1에서 2.6/153.6 µs, K=2에서 7.9/356.1 µs, K=3에서 58.6/651.3 µs입니다. 가장 큰 관측 최대값은 20 Hz의 50 ms 주기 대비 약 1.30%입니다. 이는 유한 표본 측정이지 증명된 WCET나 배포 latency의 상한이 아닙니다. 코어는 𝒮_k를 만들기 전에 모든 열거 후보를 평가하고 작업량은 입력 사람 전체·rollout 표본·환경 질의에도 의존합니다. 명시 라벨 차원만 제한해서 이 모든 비용을 제한하지는 못합니다. 이 역사적 측정은 새 후보 어댑터·실 costmap·콜백·TF·controller dispatch·DDS/executor 지터까지의 ROS 2 pipeline을 측정한 것이 아닙니다. 이들의 시간과 실제 장면의 안전집합 분포는 별도 평가 대상입니다.

### 5.5 민감도 분석 설계

proxemics·관습 파라미터의 민감도를 다음과 같이 설계합니다. 이 가운데 진행 경화의 ρ-구동률 축은 5.3에서 오프라인 전환 봉쇄 구간 `v_lat ∈ (0.20, 0.35) m/s`로 측정했으며, 물리 freezing과 나머지 축의 측정은 계획된 평가(5.6)에 남습니다.
...
- **e_max·λ·k_ρ.** 도전 누적기의 양측 조건(대수적 헤드룸 `e^rev_max > E_0` — 4.9, 봉쇄 존재 `e^rev_max < E_0(1+k_ρ)`·`ρ̄ < 1` — P4) 위반·만족 구간에서 전환율·윈드업·잠금 거동을 확인합니다. 특히 `λ`가 도달 상한 `e^rev_max = min(e_max, (D_max−Δ_floor)dt/(1−λ))`를 통해 봉쇄 문턱 `ρ̄`를 어떻게 이동시키는지를 측정합니다. **노브 튜닝 지침(봉쇄 vs 전환 가능성 동시 만족).** 4.6의 비공허성 논의대로, 누수 평형 상한 `(D_max−Δ_floor)dt/(1−λ)`가 `E_0` 아래로 떨어지면 도달 상한이 `E_0`에 닿지 못해 어떤 재량 전환도 불가능해지고 P1·P2 전환 동역학이 공허해집니다. 예컨대 `D_max=0.5, Δ_floor=0.05, dt=0.05, λ=0.7`이면 누수 평형 상한은 `(0.5−0.05)·0.05/(1−0.7) ≈ 0.075`로 `E_0=0.3`보다 작아, 짧은 `dt`·작은 `Δ_floor`·`λ<1` 영역에서 전환 능력이 의도치 않게 사라질 수 있습니다. 따라서 본 스윕은 설계자가 봉쇄(`e^rev_max < E_0(1+k_ρ)`)와 전환 가능성(`e^rev_max > E_0`)을 동시에 만족하는 `(λ, e_max)` 영역을 식별하도록, 도달 상한이 `(E_0, E_0(1+k_ρ))` 구간에 드는 `λ`·`e_max` 조합을 명시적으로 표시합니다.
...
본 논문은 사회적 주행에서 통과 결정의 시간 일관성을 1급 목표로 삼아, `class`의 ID-색인 표현·주기 간 대응과 누수 증거 누적 기반 커밋 전환 규칙, 그리고 안전 사전식 오버라이드를 결합한 결정 계층 플래너를 제안하였습니다. 다섯 속성(P1–P5)을 정형화하고, P1은 i.i.d. 도전자 우위라는 조건 아래 pre-test 증거의 주기별 교차확률 상한으로, P4는 진행도 `ρ`의 동역학과 횡 진행률 하한 및 **단일 도전 누적기 `e^rev`의 봉쇄 조건** 위에서 제시하였습니다. P2와 구분되는 별도 응답시간 상한은 동일한 안전·가용 도전자의 지속 선택, 중단 없는 갱신, 지속 우위 및 clip/leak 평형 헤드룸을 전제로 합니다. 실증 측면에서는 실제 결정 코어를 구동하는 오프라인 하니스로 반진동, 가독성 프록시, 결정 코어 지연을 측정했으며(§5.3·§5.4), 진행 경화의 오프라인 전환 봉쇄 구간 `v_lat∈(0.20,0.35) m/s`를 확인했습니다. 이는 물리 freezing 임계가 아닙니다.

**한계.** (i) 평면(2D) 가정으로, 다층·경사 환경은 다루지 않습니다. (ii) 칼만-등속 예측은 단기에 강하나 급격한 의도 변화·다중 모드 군중에는 한계가 있습니다. (iii) 앞/뒤 통과를 비용으로만 처리하고 별도 `class`로 분리하지 않습니다. (iv) 실증 범위는 결정 계층에 한정됩니다. 행동 일관성·가독성 프록시·주기 지연은 실제 결정 코어를 구동하는 오프라인 하니스로 측정했지만, 물리 성공률·충돌·최소 사회 거리·freezing, 외부 비교군 및 사용자 실험은 아직 측정하지 않았습니다. `v_lat∈(0.20,0.35) m/s`는 스크립트형 입력에서의 전환 봉쇄 구간일 뿐 물리 freezing 임계가 아닙니다. 따라서 현재 기여는 결정 규칙·정형 속성과 **결정 계층 일관성의 오프라인 실증**이며, 물리 성과·외부 비교·사람 데이터 검증은 §5.6의 프로토콜에 남습니다.
...
**소프트웨어·증거 버전.** 통합 구현은 ROS 2 Jazzy, 계획된 물리 시뮬레이션은 Gazebo Harmonic을 대상으로 합니다. ROS 빌드·시험 근거와 실험 provenance는 구분합니다. R3는 AMD Ryzen 5 5500GT의 역사적 결정 코어 latency이며 새로운 통합 컨트롤러·ROS 파이프라인 시간 측정이 아닙니다. 현 R1 관찰자 출력은 AMD EPYC 9V74에서 다시 계산했습니다. PDF의 기준 소스는 `paper/arxiv/main.tex`이고 `scripts/build_paper.sh`는 TeX Live(science/extra 포함)의 pdfLaTeX, Poppler, ripgrep을 사용합니다. `--check`는 임시 디렉터리 빌드이며 `--write`는 PDF와 해시 manifest를 의도적으로 갱신합니다. 별도 시각 QA가 필요합니다. 새 실행에서는 실제 컴파일러·표준 라이브러리·TeX/패키지 버전·하드웨어·플래그·명령을 기록해야 합니다.

**빌드·실행 명령.** 저장소 루트에서 실행합니다. 전체 빌드는 ROS 2 Jazzy 환경과 명시된 Nav2·시뮬레이터 종속성이 필요하며, evaluator만 빌드할 때는 `--base-paths src --packages-up-to compass_eval`로 대체할 수 있습니다. 아래 명령은 기존 측정을 보존합니다.

    # Full workspace, including simulator package (not a simulator run)
...
| `E0` | 0.30 | 누적기 기준 임계 `E_th(0)` (정규화 비용×초) |
| `k_rho` | 1.0 | 진행 경화 계수 `k_ρ` |
| `p` | 2 | 진행 경화 지수 |
| `lambda` | 0.97 | 누수 계수 `λ` (τ_leak≈1.64 s) |
| `e_max_rev` | 0.50 | 증거 clip 상한 `e_max` (정규화 비용×초) |
...
| `K_cap` | 3 | 명시 차원 상한 |
| `a_brake` | 0.5 | 가속도 상한(m/s²); 속도 감소량 `a_brake·dt` |
...
**소스·결과 provenance.** 이 원문은 `https://github.com/kjungmo/compass`의 검토 중인 통합 후보와 함께 제공되며 이미 main에 병합되었다고 주장하지 않습니다. 새 실행마다 실제 checkout commit 및 소스·설정·입력 해시를 기록합니다. 원본 결과는 `9fe495a`, 중간 log-odds-only 관찰자는 PR12 `5343d457`에서 보존되며, 현 R1은 별도의 0.30초 후속 관측 규칙을 추가합니다. 버전 기록은 `src/compass_eval/results/README.md`를 따릅니다. 현재 행동 값은 `ablation_raw.csv`·`R1_ablation.md`, 역사적 범위가 구분되는 측정은 `R3_latency.md`·`R5_rho_sweep.md`에 있습니다. 개념 그림은 실측 궤적이 아니라 해당 스크립트에서 생성합니다. `check_observer_results.py`의 라벨별 셀 검사와 `check_paper_numbers.py`의 수치 검사는 선언된 검사 범위만 검증하며 모든 과학 주장을 증명하지 않습니다. `paper/arxiv/artifact_manifest.json`은 빌드된 원문/PDF/그림의 해시를 기록하고, 제출 tag는 별도의 향후 release 행위입니다.

-----

## 역사적 개정 기록 — 아래 내용은 현재 주장 아님

아래 v0.4–v0.8 기록에는 이후 철회된 두 누적기·상관 입력 지수 상한·비편향 관찰자·경계 등호 주장도 포함됩니다. 연구 이력의 보존을 위한 스냅샷이며 현재 조건은 위 §4.6과 영문 기준 문서를 따릅니다.
...
- **[v0.6-4] (5.8 R5 전환 봉쇄):** ρ-구동률(v_lat) 민감도 실측으로 오프라인 전환 봉쇄 구간 v_lat∈(0.20,0.35) m/s를 보고. 물리 freezing과 구분하고, 측정 진행도 분리의 근거로 한정.
...
- **[CLAIM-1] (초록):** 오프라인 전환 봉쇄 구간 v_lat∈(0.20,0.35) m/s를 물리 freezing 임계와 구분하고, 물리 성과는 계획된 평가로 남김.
...
- **[CLAIM-4] (§5.3):** `mid_reversal`에서 제안의 무전환이 단순 드웰과 동일한 가림 효과의 이면임을 명시하고, 이를 오프라인 전환 봉쇄 민감도와 연결.
...
- **[Round 5 observer 수리]** 정책·시나리오를 유지하고 posterior를 log-odds로 계산했습니다. 과거 이력의 intermittent 50/50 검열은 수치 오류로 폐기되며, 현재 표와 raw CSV가 교정 결과입니다. 행동 재실행 CPU: AMD EPYC 9V74; R3 지연은 기존 Ryzen 5 5500GT 측정입니다.
