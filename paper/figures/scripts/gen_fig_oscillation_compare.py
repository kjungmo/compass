...
VARIANT_LABEL_EN = {
    ...
    "simple-dwell (O4)": "Simple dwell (O4 baseline)",
...
def main():
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(labels, fontsize=9, rotation=18, ha="right", rotation_mode="anchor")
    ...
    for xi, m, n in zip(x, means, ns):
        ax.text(xi, max(m, plot_floor) * 1.35, f"{m:.2f}",
