...
micromamba run -n ros2 bash -lc '
  set +u
  source ~/kangj/compass/install/setup.bash
  export MESA_GL_VERSION_OVERRIDE=3.3        # WSL2 소프트웨어 GL 폴백
...
micromamba run -n ros2 bash -lc '
  ...
  gz sim -r -s --headless-rendering --iterations 200 \
    ~/kangj/compass/sim/worlds/compass_office.sdf
'
```
...
이 워크스테이션은 **WSL2** 이며, RoboStack `libGL` 과 호스트 드라이버 충돌로
Gazebo 의 렌더링/lidar 가 느려지거나 멈출 수 있습니다(형제 repo `se-mppi-nav2`
에서 실측). 따라서:
